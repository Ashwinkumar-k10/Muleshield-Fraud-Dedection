...
target_paper = 'MuleShield_PRO_IEEE_Paper_FINAL_SYNCHRONIZED.docx'
other_papers = ['MuleShield_PRO_IEEE_Paper_FINAL.docx', 'MuleShield_PRO_IEEE_Paper (1).docx']

# Exact documented canonical values from docs/12_MODEL_EVALUATION.md
doc_metrics = {
    "pr_auc": "0.8807",
    "pr_auc_std": "0.0403",
    "pr_auc_full": "0.8807 ± 0.0403",
...
def synchronize_docx(src_path, dst_path):
    ...
    # Replace 0.9180 / 0.9131 / 59.26% / 0.7442 claims with exact documented 0.8807 ± 0.0403 metrics
    full_xml = full_xml.replace(
        ...
        'Under strict leakage-free five-fold group-aware cross-validation across all 9,082 accounts, the final verified model achieves a PR-AUC of 0.8807  0.0403 (with a hyperparameter search champion result of 0.8833  0.0365). At the selected operating threshold of 0.9899, the system reaches 100.00% precision and 61.64% recall (F1 = 0.7586)'
    )
    ...
    full_xml = full_xml.replace(
        ...
        'The final verified model achieved a five-fold group-aware cross-validation PR-AUC of 0.8807  0.0403 (hyperparameter search champion: 0.8833  0.0365); at the selected decision threshold of 0.9899 the system reached 100.00% precision and 61.64% recall (F1 = 0.7586).'
    )
    ...
    # Update Table VI (Canonical Final Performance -> Verified 5-Fold Group CV Performance)
    # Table VI is index 5
    if len(tables) > 5:
        t6 = tables[5]
        t6_rows = [
            ...
            ("Validation Precision", "100.00%", "Threshold 0.9899 (0 False Positives)"),
            ...
            ("In-Sample Full-Dataset Specificity", "99.989%", "Diagnostic Inference (9,000 TN, 1 FP)"),
            ("In-Sample Full-Dataset Sensitivity", "67.901%", "Diagnostic Inference (55 TP, 26 FN)"),
    ...
    # Update Table VII (Threshold Analysis Table)
    # Table VII is index 6
    if len(tables) > 6:
        t7 = tables[6]
        t7_rows = [
    ...
    print(f"Successfully generated synchronized docx: {dst_path}")

# Run synchronization
synchronize_docx(backup_path, target_paper)
for p in other_papers:
    try:
        synchronize_docx(backup_path, p)
