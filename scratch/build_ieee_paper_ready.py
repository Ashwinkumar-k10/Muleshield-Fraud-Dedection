...
ready_paper = 'MuleShield_PRO_IEEE_Paper_FINAL_READY.docx'
other_papers = [
    'MuleShield_PRO_IEEE_Paper_FINAL_SUBMISSION.docx',
...
def generate_ready_docx(src_path, dst_path):
    ...
    # 1. Terminology alignment & claim polish
    ...
    # 2. Quantitative alignment in Abstract & Intro
    full_xml = full_xml.replace(
    ...
    print(f"Successfully generated ready submission docx: {dst_path}")

# Run generation
generate_ready_docx(backup_path, ready_paper)
for p in other_papers:
    try:
        generate_ready_docx(backup_path, p)
