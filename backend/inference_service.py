...
def health():
    return jsonify({"service": "ml-inference", "status": "UP"}), 200


@app.route("/ready", methods=["GET"])
def ready():
    if risk_engine is not None and load_error is None:
        return jsonify({"service": "ml-inference", "status": "READY"}), 200
    return jsonify({
        "service": "ml-inference",
        "status": "NOT_READY",
        "error": load_error or "Inference engine not initialized"
    }), 503
...
def predict():
    ...
    if risk_engine is None:
        return jsonify({
            "error": "Inference engine is not ready",
            "detail": load_error
        }), 503
    ...
    try:
        ...
        return jsonify({
            "error": "Inference calculation failed",
            "detail": str(e)
        }), 500
...
if __name__ == "__main__":
    ...
    logger.info(f"Starting ML Inference Service on http://0.0.0.0:{port} ...")
